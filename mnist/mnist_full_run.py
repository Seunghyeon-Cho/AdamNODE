...
def model_gen(name, **kwargs):
    if name == 'node':
        ...
        dim = 5
        nhid = 50
        layer = NODElayer(AdamNODEs(DF(dim, nhid), None), **kwargs)
        model = nn.Sequential(adamnode_initial_velocity(1, dim, nhid),
                              layer, predictionlayer(dim, truncate=True)).to(device=args.gpu)
    else:
...
if __name__ == '__main__':
    ...
    rec_names = ["model", "test#", "train_test", "iter", "loss", "acc", "forwardnfe", "backwardnfe", "time/iter",
                 "time_elapsed"]
